<template>
  <div class="layout-content">
    ...
    <div class="category-layout">
      ...
      <div class="category__line" v-if="categoryData.vodAreaData && categoryData.vodAreaData.length > 0">
    ...
    <div class="lvideo-list" >
      <a class="video-item" :href="handleDetail(item.id)" v-for="item in tData.vodData" :key="item.id"  @click="handleItemClick(item)">
        ...
        <div class="cover-wrap">
          ...
          <span class="remarks">{{ item.video_episode }}</span>
        </div>
        <div class="meta-wrap">
...
const goToDetailPage = (item) => {
  ...
  localStorage.setItem('classification',"电视剧"); //埋点上报分类
...
const fetchVodClassData = async () => {
  try {
    const response = await axios.get('https://video.lieyuan.cc/api/videov2/catrgories');
    const data = response.data.data;
    vodClassData.value = data.类型;
    vodAreaData.value = data.地区;
    vodlanguageData.value = data.language;
    console.error('vodAreaData.value:', response.data.data);
...
const getData = function (categoryID) {
  ...
  axios.get('https://video.lieyuan.cc/api/videov2/videos', {
    params: filterDict
  })
      .then(response => {
        console.log('red',response.data.data.records);
        tData.vodData.push(...response.data.data.records); // 将新数据追加到现有数据列表
      })
      .catch(error => {
